...
## INSERT ... ON CONFLICT DO SELECT RETURNING — PG 19
...
```sql
CREATE INDEX ON bluebox.rental_schedule
    USING gist (inventory_id, rental_period);
-- One index handles both:
--   inventory_id    = 42         (B-tree equality)
...
- Fuses B-tree equality and GiST range ops into one index
...
- **2-3x gains are cold-cache seq scans** on cloud storage (EBS, Azure, GCP PD)
- **Local NVMe?** Already microsecond latency — smaller gains
- **Reads only** — writes and WAL still synchronous

**The real story:** 5 years of engineering to rebuild the I/O path. Async writes, WAL, and Direct I/O are next.
...
- Especially effective when leading column has low cardinality
- No new indexes needed, existing composite indexes just work better
...
**Before PG 19:** Recursive CTEs — powerful but not really graphs - REVERTED SEPT 7


**PG 19:** SQL/PGQ — ISO SQL:2023 standard graph queries over your existing tables

**Extension:** Apache AGE — full Cypher (Neo4j query language) inside Postgres

---

## CREATE PROPERTY GRAPH (PG 19)
