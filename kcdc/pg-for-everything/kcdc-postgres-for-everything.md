...
- No **pattern subscriptions** (Redis has PSUBSCRIBE)
...
| Postgres LISTEN/NOTIFY |  Use Kafka/Redis when... |
|--------------------------|--------------------------|
| Signaling: cache invalidation | High throughput (>1K msg/sec) |
| Job wakeup triggers | Message durability required |
| Live UI refresh | Multi-consumer replay |
| Zero extra infrastructure | Cross-service event bus |
| Transactional delivery | Pattern subscriptions needed |
...
- **Triggers + outbox table** — transactional outbox pattern
...
|  Postgres UNLOGGED |  Use Redis when... |
|---------------------|---------------------|
| Moderate read loads | Sub-ms latency critical |
| Already have Postgres | Millions of ops/sec |
| Same backup/monitoring | Built-in eviction policies |
| JSONB flexibility | Distributed cache needed |
| Transactional with app data | Pub/sub on cache keys |
...
# SQL/PGQ: 2 Hops — Kevin Bacon to Sally Field
...
# PG19 SQL/PGQ vs Apache AGE

- **SQL/PGQ**: graph is a view over your tables — zero ETL, zero sync
- **Apache AGE**: separate graph store — must load data, but gets variable-length paths
- For fixed-depth patterns (1-2 hops): **SQL/PGQ is simpler**
- For deep traversals or "find any path": **AGE (or Neo4j) is necessary**
- Both run inside Postgres — no separate database server

---

# Blog: "PostgreSQL 19 Is a Graph Database Now"

> "If you have ever spun up Neo4j because you had a vague intuition that your data was graph-shaped, this is the release that lets you reconsider."
> — thebuild.com (Christophe Pettus)

^ https://thebuild.com/blog/sqlpgq-in-postgresql-19-graph-queries-without-the-graph-database/ — Christophe Pettus is the CEO of PostgreSQL Experts, Inc. and a well-known PG community figure. Also see: https://www.kynodex.com/blog/postgresql-19-is-a-graph-database-now-and-you-dont-need-neo4j
...
| Postgres Analytics |  Use ClickHouse/DuckDB when... |
|---------------------|-------------------------------|
| Mixed OLTP + analytics | Pure analytical workloads |
| Rollups will help | Columnar compression needed |
...
1. Your scale is moderate (< millions of events/sec)
2. You value operational simplicity
3. Your data has relational connections
4. You need ACID guarantees
5. One less service = one less failure mode
...
1. You're hitting **millions of ops per second**
2. You need **distributed, multi-region** by default
3. The specialized tool has **10x better performance** for your exact workload
4. Your team **already has expertise** in the specialized tool
5. The problem is **the entire product** (search engine, not "has search")
